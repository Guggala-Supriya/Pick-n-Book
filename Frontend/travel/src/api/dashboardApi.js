...
const DASHBOARD_ROOT = "/api/Dashboard";
...
function normalizeDashboardSummary(payload) {
  ...
  const flightBookings = pickFirst(
    safePayload,
  ...
  return {
    ...
    flightBookings: {
      completed: toNumber(pickFirst(flightBookings, ["completed", "Completed"], 0)),
      upcoming: toNumber(pickFirst(flightBookings, ["upcoming", "Upcoming"], 0)),
      cancelled: toNumber(pickFirst(flightBookings, ["cancelled", "Cancelled"], 0)),
      total: toNumber(pickFirst(flightBookings, ["total", "Total"], 0)),
    },
    busBookings: {
